...
function dsprintf() {
	...
	// diff the data with the used_keys
...
function dsprintfMatch($m1, $m2, &$data, &$used_keys) {
	if (isset($data[$m1])) {// if the key is there
		$str = $data[$m1];
		$used_keys[$m1] = $m1;
		// dont unset it, it can be used multiple times
		return sprintf("%" . $m2, $str);
		// sprintf the string, so %s, or %d works like it should
	} else {
		return "%" . $m2;
		// else, return a regular %s, or %d or whatever is used
	}
}
...
class FX {
	...
	public function __construct() {
		...
		$this -> _keywordmap = array('checkbox' => array('is', 'has', 'opt'), 'color' => array('color', 'colour'), 'date' => array('date'), 'datetime' => array('start', 'starts', 'end', 'ends'), 'email' => array('email'), 'file' => array('file'), 'hidden' => array('key', 'secret'), 'image' => array('pic', 'image', 'img'), 'month' => array('month'), 'number' => array('num', 'number', 'id'), 'password' => array('password'), 'select' => array('type', 'country', 'state', 'county'), 'range' => array('range'), 'radio' => array('or', 'sex', 'gender'), 'search' => array('search', 'term', 'query'), 'tel' => array('tel', 'phone'), 'time' => array('time'), 'url' => array('url', 'link'), 'week' => array('week'), 'textarea' => array('note', 'description'));
		...
		$this -> styles = array('form-divs' => array('label' => '<label for="%(name)s">%(label)s</label>', 'text' => '<input type="%(itype)s" name="%(name)s" id="%(name)s" placeholder="%(placeholder)s" />', 'textarea' => '<textarea name="%(name)s" id="%(name)s" placeholder="%(placeholder)s" /></textarea>', 'open' => '<div>', 'close' => '</div>', 'class' => 'form-divs', 'submit' => '<div><button type="submit">%s</button></div>', 'formopen' => '<form class="%s" action="%s"><fieldset>'), 'form-horizontal' => array('label' => '<label for="%(name)s">%(label)s</label>', 'text' => '<div class="controls"><input type="%(itype)s" id="%(name)s" name="%(name)s" placeholder="%(placeholder)s"></div>', 'textarea' => '<div class="controls"><textarea id="%(name)s" name="%(name)s" placeholder="%(placeholder)s"></textarea></div>', 'open' => '<div class="control-group">', 'close' => '</div>', 'class' => 'form-horizontal', 'submit' => '<div class="control-group"><div class="controls"><button type="submit" class="btn">%s</button></div></div>', 'formopen' => '<form class="%s" action="%s"><fieldset>'));
	...
	public function addField($_field) {
		...
		if (gettype($_field) == "array" and !isset($_field['itype'])) {
			...
			$field['multiple'] = (isset($_field['multiple']) and $_field['multiple'] == TRUE) ? 'multiple' : '';
			$field['checked'] = (isset($_field['checked']) and $_field['checked'] == TRUE) ? 'checked' : '';
	...
	public function getLabel($field) {
		return dsprintf($this -> styles[$this -> style]['label'], $field);
	}
	...
	public function getDefaultInput($field) {
		return dsprintf($this -> styles[$this -> style]['text'], $field);
	}

	public function getTextArea($field) {
		return dsprintf($this -> styles[$this -> style]['textarea'], $field);
	}

	public function getCheckboxInput($field) {

		if ($this -> style == 'form-divs') {
			return dsprintf('<input type="checkbox" name="%(name)s" id="%(name)s" %(checked)s /> %(note)s', $field);
		}
